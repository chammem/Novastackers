...
router.get('/best-driver', async (req, res) => {
  ...
  router.get('/test-driver-service/:orderId', async (req, res) => {
    ...
    try {
      ...
      process.stdout.write(`[DIRECT TEST] Has delivery address: ${Boolean(order.deliveryAddress)}\n`);
      process.stdout.write(`[DIRECT TEST] Has coordinates: ${Boolean(order.deliveryAddress?.lat && order.deliveryAddress?.lng)}\n`);
      ...
      process.stdout.write("[DIRECT TEST] Calling driver assignment service...\n");
      ...
      process.stdout.write(`[DIRECT TEST] Service returned: ${JSON.stringify(result)}\n\n`);
