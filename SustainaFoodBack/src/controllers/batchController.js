...
exports.generateBatches = async (req, res) => {
  try {
    ...
    const itemsWithCoordinates = foodItems.filter(item => 
      item.buisiness_id && 
    ...
    if (itemsWithCoordinates.length === 0) {
      return res.status(400).json({
        message: "Cannot create batches - no food items have valid location data"
      });
    }
    ...
    res.status(500).json({ message: "Error generating batches", error: error.message });
  }
};
...
exports.getAvailableVolunteersForBatch = async (req, res) => {
  try {
    ...
    if (!campaign || !campaign.volunteers || campaign.volunteers.length === 0) {
      return res.status(200).json({ message: "No volunteers found", volunteers: [] });
    }
    ...
    const availableVolunteers = campaign.volunteers.filter(volunteer => {
      // Check availability
      if (!volunteer.availability || volunteer.availability.size === 0) return false;
    ...
    availableVolunteers.sort((a, b) => {
      ...
      return Math.abs(aCapacity - requiredCapacity) - Math.abs(bCapacity - requiredCapacity);
    });
    
    return res.status(200).json({
      message: availableVolunteers.length > 0 ? "Found available volunteers" : "No volunteers match criteria",
    ...
    res.status(500).json({ message: "Error finding volunteers", error: error.message });
  }
};
...
exports.assignVolunteerToBatch = async (req, res) => {
  try {
    ...
    console.log(`[BATCH ASSIGN] Starting batch request process for batch ${batchId} to volunteer ${volunteerId}`);
    ...
    console.log(`[BATCH ASSIGN] Found batch with ${batch.items.length} items, status: ${batch.status}`);
    ...
    await batch.save();
    console.log(`[BATCH ASSIGN] Batch saved with assignedVolunteer=${volunteerId}, status=requested, assignmentStatus=pending`);
    ...
    console.log(`[BATCH ASSIGN] Updated ${updateResult.modifiedCount}/${batch.items.length} food items to requested status`);
    
    // Create notification
    console.log(`[BATCH ASSIGN] Creating notification for volunteer ${volunteerId}`);
    ...
    console.log(`[BATCH ASSIGN] Notification created with ID: ${notification._id}`);
    
    // Socket notification
    console.log(`[BATCH ASSIGN] Socket available?`, req.io ? "YES" : "NO");
    if (req.io) {
      console.log(`[BATCH ASSIGN] Emitting notification to room: ${volunteerId.toString()}`);
      ...
      console.log(`[BATCH ASSIGN] Socket notification skipped - req.io not available`);
    }
    
    res.status(200).json({ message: "Batch assignment requested. Volunteer will be notified.", batch });
  } catch (error) {
    console.error(`[BATCH ASSIGN] ERROR:`, error);
    res.status(500).json({ message: "Error requesting batch assignment", error: error.message });
  }
};
...
exports.acceptBatchAssignment = async (req, res) => {
  try {
    ...
    if (batch.assignedVolunteer.toString() !== volunteerId.toString()) {
      return res.status(403).json({ message: "This batch is not assigned to this volunteer" });
    }
    
    if (batch.status !== 'requested' || batch.assignmentStatus !== 'pending') {
      return res.status(400).json({ message: "This batch is not in a requestable state" });
    }
    ...
    if (campaign && campaign.ngoId) {
      ...
      if (req.io) {
        req.io.to(campaign.ngoId.toString()).emit("new-notification", notification);
      }
    }
    ...
    res.status(500).json({ message: "Failed to accept batch assignment", error: error.message });
  }
};
...
exports.declineBatchAssignment = async (req, res) => {
  try {
    ...
    res.status(500).json({ message: "Failed to decline batch assignment", error: error.message });
  }
};
